...
export class SetPage extends Control {
  ...
  constructor(parentNode: HTMLElement) {
    ...
    new Control(this.node, 'h2', 'h2 display-2 text-center', 'Настройки');
    ...
    setData.forEach((el: SettingsData, index: number) => {
      ...
      if (index !== setData.length - 1) {
        new Switcher(innerLeft.node, data);
      } else {
        new SoundEl(innerRight.node, data);
      }
    });

    new ChangePassword(innerRight.node);
    new ChangeName(innerRight.node);
    ...
    btnReset.node.onclick = (): void => this.resetSettings();
    ...
    btnSave.node.onclick = (): void => this.saveSettings();
